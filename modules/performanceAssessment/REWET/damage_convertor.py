...
def createPipeDamageInputForREWET(pipe_damage_data, run_dir, event_time):
    """
    ...
    for pipe_id in pipe_id_list:
        ...
        segment_sizes = len( cur_damage["aggregate"] )
        segment_step = 1 / segment_sizes
        c = 0
        
        for damage_val in cur_damage["aggregate"]:
            if damage_val > 0:
                if damage_val == 1:
                    damage_type = "leak"
                elif damage_val == 2:
...
def createNodeDamageInputForREWET(node_damage_data, REWET_input_data):
    """
...
def readDamagefile(file_addr, run_dir, event_time):
    """
    ...
    try:
        ...
        pass
    # wn_damage_data = damage_data["WaterNetwork"]
    wn_damage_data = {}
    if "Pipes" in wn_damage_data:
        pipe_damage_data = createPipeDamageInputForREWET(
            wn_damage_data["Pipes"], run_dir, event_time)
    else:
        pipe_damage_data = pd.Series(dtype="O")
    
    if "Tanks" in wn_damage_data:
        tank_damage_data = createTankDamageInputForREWET(
            wn_damage_data["Tanks"],  run_dir, event_time)
    else:
        tank_damage_data = pd.Series(dtype="O")
    
    if "Pumps" in wn_damage_data: 
        pump_damage_data = createPumpDamageInputForREWET(
            wn_damage_data["Pumps"],  run_dir, event_timea)
    else:
        pump_damage_data = pd.Series(dtype="O")
    
    if "Nodes" in wn_damage_data: 
        node_damage_data = createNodeDamageInputForREWET(
            wn_damage_data["Pumps"],  run_dir, event_time)
    else:
