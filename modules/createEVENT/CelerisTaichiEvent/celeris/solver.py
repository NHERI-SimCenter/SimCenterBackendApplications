...
from celeris.utils import Reconstruct,CalcUV,sineWave,CalcUV_Sed,CalcUV_Sed,NumericalFlux,FrictionCalc
...
class Solver:
    def __init__(self,
                 ...
                 sedC1_shields = 308.8993914681988,
                 sedC1_erosion = 0.0002746401358265295,
                 sedC1_fallvel = 0.14690813456034352,
                 sedC1_n = 0.4,
        ...
        self.useBreakingModel=useBreakingModel # inlcude breaking model when == 1
        ...
        self.F_G_star_oldGradients = self.domain.states()       #stores F* and G* at previous time step
        self.F_G_star_oldOldGradients = self.domain.states()    #stores F* and G* at 2*previous time step
        self.F_G_star_predictedGradients = self.domain.states() #stores F* and G* at predictor step
    ...
    @ti.kernel
    def remove_single_point_islands(self):
            ...
            for j in range(1, self.ny - 1):
                for i in range(1, self.nx - 1):
                    if self.Bottom[2,j,i] >= 0.0:
                        if (self.Bottom[2,i + 1, j] <= 0.0 and 
                            self.Bottom[2,i - 1, j] <= 0.0 and 
                            self.Bottom[2,i, j + 1] <= 0.0 and 
                            self.Bottom[2,i, j - 1] <= 0.0):
    ...
    def fill_bottom_field(self):
        ...
        for i,j in ti.ndrange((0,self.nx), (0,self.ny)):
            ...
            self.Bottom[0,j,i] = 0.5 * self.Bottom[2,j,i] + 0.5 * self.Bottom[2,min(self.ny - 1, j + 1),i]
            self.Bottom[1,j,i] = 0.5 * self.Bottom[2,j,i] + 0.5 * self.Bottom[2,j,min(self.nx - 1, i + 1)]
            # boolean near-dry check
            for yy in range(max(j - lengthCheck, 0), min(j + lengthCheck + 1, self.ny)):
                    for xx in range(max(i - lengthCheck, 0), min(i + lengthCheck + 1, self.nx)):
    ...
    @ti.func
    def WavesGenerator(self,NumWaves,Waves,x,y,t,d_here):
        result = ti.Vector([0.0, 0.0, 0.0])
        for w in range(NumWaves):
            result += sineWave(x,y,t,d_here,Waves[w,0],Waves[w,1],Waves[w,2],Waves[w,3])
        return result
    ...
    @ti.kernel
    def run_boundaries(self,time:ti.f32, TempBound: ti.template()):
        # North boundary  
        ...
        if self.bc.North ==3:                    # IrregularWave            
            for i,j in ti.ndrange((0,self.nx),(self.ny-3,self.ny)):
                ...
                eta,hu,hv = 0.,0.,0.
                if d_here > 0.0001:
                    eta,hu,hv = self.WavesGenerator(self.Nwaves,self.WaveData,x,y,time,d_here)
        ...
        if self.bc.South ==3:                    # IrregularWave            
            for i,j in ti.ndrange((0,self.nx),(self.ny-3,self.ny)):
                d_here = max( 0 , self.nSL -self.Bottom[2,j,i])
                x = i * self.dx
                y = j * self.dy
        ...
        if self.bc.East ==3:                    # IrregularWave            
            for i,j in ti.ndrange((0,self.nx),(self.ny-3,self.ny)):
                d_here = max( 0 , self.nSL -self.Bottom[2,j,i])
                x = i * self.dx
                y = j * self.dy
        ...
        if self.bc.West ==3:                    # IrregularWave            
            #for i,j in ti.ndrange((0,3),(0,self.ny)):
            for i in range(0,3):
                for j in range(0,self.ny):
                    
                    B_here = -self.maxdepth
        ...
        if self.bc.West ==4 and time==0:                    # ~Dam break
    ...
    @ti.kernel
    def bottom_derivatives(self):
        for i,j in ti.ndrange((1,self.nx-1),(1,self.ny-1)):
            self.bottom_dxdy[j,i][0] = ( -self.Bottom[2,j,i+1] + self.Bottom[2,j,i-1])/(2*self.dx)
            self.bottom_dxdy[j,i][1] = ( -self.Bottom[2,j+1,i] + self.Bottom[2,j-1,i])/(2*self.dy)

    @ti.kernel
    def tridiag_coeffs(self):
        ...
        for i,j in ti.ndrange((2,self.nx-2),(2,self.ny-2)):
            ...
            self.coefMatx[j,i][0] = d * d_dx/(6.0*self.dx) - (self.Bcoef + 1./3)*d**2/self.dx**2
            self.coefMatx[j,i][1] = 1.                    +2*(self.Bcoef + 1./3)*d**2/self.dx**2
            self.coefMatx[j,i][2] =-d * d_dx/(6.0*self.dx) - (self.Bcoef + 1./3)*d**2/self.dx**2
            if i==2:  #P_3=0  % for solid wall, coefMatx(i,j,3) will be set to zero, for wave input it will be set to wave velocity
                self.coefMatx[j,i][0] = 0
                self.coefMatx[j,i][1] = 1
                self.coefMatx[j,i][2] = 0
            elif i==self.nx-2: #P_nx-2=0
                self.coefMatx[j,i][0] = 0
                self.coefMatx[j,i][1] = 1
                self.coefMatx[j,i][2] = 0
                
            self.coefMaty[j,i][0] = d * d_dy/(6.0*self.dy) - (self.Bcoef + 1./3)*d**2/self.dy**2
            self.coefMaty[j,i][1] = 1.                    +2*(self.Bcoef + 1./3)*d**2/self.dy**2
            self.coefMaty[j,i][2] =-d * d_dy/(6.0*self.dy) - (self.Bcoef + 1./3)*d**2/self.dy**2
            if j==2:
    ...
    def tridiag_coeffs_X(self):
        ...
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            ...
            if i <= 2 or i >= (self.nx - 3) or neardry < 0.0:
                  ...
                  a = depth_here * d_dx / (6.0 * self.dx) - (self.Bcoef + 1.0 / 3.0) * depth_here * depth_here / (self.dx * self.dx)
                  b = 1.0 + 2.0 * (self.Bcoef + 1.0 / 3.0) * depth_here * depth_here / (self.dx * self.dx)
                  c = -depth_here * d_dx / (6.0 * self.dx) - (self.Bcoef + 1.0 / 3.0) * depth_here * depth_here / (self.dx * self.dx)
    ...
    def tridiag_coeffs_Y(self):
        ...
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            ...
            if j <= 2 or j >= self.ny - 3 or neardry < 0.0:
                  ...
                  # Calculate coefficients
                  a = depth_here * d_dy / (6.0 * self.dy) - (self.Bcoef + 1.0 / 3.0) * depth_here * depth_here / (self.dy * self.dy)
                  b = 1.0 + 2.0 * (self.Bcoef + 1.0 / 3.0) * depth_here * depth_here / (self.dy * self.dy)
                  c = -depth_here * d_dy / (6.0 * self.dy) - (self.Bcoef + 1.0 / 3.0) * depth_here * depth_here / (self.dy * self.dy)
    ...
    def Pass1(self):
        ...
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            ...
            rightIdx = min(pi + 1, self.nx - 1)
            upIdx = min(pj + 1, self.ny - 1)
            leftIdx = max(pi - 1, 0)
            downIdx = max(pj - 1, 0)
             
            #Read in the state of the water at this pixel and its neighbors [eta,hu,hv,c]
            in_here = self.State[pj, pi]
            in_S = self.State[downIdx, pi]
            in_N = self.State[upIdx, pi]
            in_W = self.State[pj, leftIdx]
            in_E = self.State[pj, rightIdx]
                        
            B_here = self.Bottom[2,pj,pi]
            B_south = self.Bottom[2,downIdx,pi]
            B_north = self.Bottom[2,upIdx,pi]
            B_west = self.Bottom[2,pj,leftIdx]
            B_east = self.Bottom[2,pj,rightIdx]
            ...
            dB_west = abs(B_here - B_west)
            dB_east = abs(B_here - B_east)
            dB_south = abs(B_here - B_south)
            dB_north = abs(B_here - B_north)
            ...
            wetdry = min(h_here, min(h_south, min(h_north, min(h_west, h_east))))
            ...
            Fr = speed / tm.sqrt(9.81 / divide_by_h)
            Frumax = max(Fr.x, max(Fr.y, max(Fr.z, Fr.w)))
            dBdx = abs(B_east - B_west) / (2.0 * self.dx)
            dBdy = abs(B_north - B_south) / (2.0 * self.dy)
            ...
            maxInundatedDepth = max((h[0] + h[1] + h[2] + h[3]) / 4, self.Auxiliary2[pj, pi][0])
            ...
            if (0.5 * tm.sign(normal[0] * in_here[1] + normal[1] * in_here[2]) > self.dissipation_threshold):
                breaking_white = 1.0
    ...
    def Pass1_SedTrans(self):
        ...
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            ...
            in_S = self.State_Sed[pj-1, pi]
            in_N = self.State_Sed[pj+1, pi]
            in_W = self.State_Sed[pj, pi-1]
            in_E = self.State_Sed[pj, pi+1]
            
            #
            B_here = self.Bottom[2,pj,pi]
            B_south = self.Bottom[2,pj-1,pi]
            B_north = self.Bottom[2,pj+1,pi]
            B_west = self.Bottom[2,pj,pi-1]
            B_east = self.Bottom[2,pj,pi+1]
            ...
            output_c1, output_c2, output_c3, output_c4 = CalcUV_Sed(h, hc1, hc2, hc3, hc4, self.epsilon, dB_max)
    ...
    def Pass2(self):
        ...
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            ...
            hW_east = self.H[pj,rightIdx][3]
            hS_north = self.H[upIdx,pi][2]
    
            u_here = ti.Vector([0.0, 0.0])
            u_here[0] = self.U[pj, pi][0]
            u_here[1] = self.U[pj, pi][1]
            
            uW_east = self.U[pj, rightIdx][3]
            uS_north = self.U[upIdx, pi][2]
    
            v_here = ti.Vector([0.0, 0.0])
            v_here[0] = self.V[pj, pi][0]
            v_here[1] = self.V[pj, pi][1]
            
            vW_east = self.V[pj, rightIdx][3]
            vS_north = self.V[upIdx, pi][2]
            
            # Compute wave speeds
            cNE = tm.sqrt(self.g * h_here)    
            cW = tm.sqrt(self.g * hW_east)     # cW evaluated at (j+1, k)
            cS = tm.sqrt(self.g * hS_north)    # cS evaluated at (j, k+1)            
            
            # Compute propagation speeds
            aplus = max(max(u_here[1] + cNE[1], uW_east + cW), 0.0)
            aminus = min(min(u_here[1] - cNE[1], uW_east - cW), 0.0)
            bplus = max(max(v_here[0] + cNE[0], vS_north + cS), 0.0)
            bminus = min(min(v_here[0] - cNE[0], vS_north - cS), 0.0)
                        
            B_here = self.Bottom[2,pj,pi]
            dB = max(self.Bottom[2,downIdx,pi] - B_here, max(self.Bottom[2,upIdx,pi] - B_here, max(self.Bottom[2,pj,leftIdx] - B_here, self.Bottom[2,pj,rightIdx] - B_here)))            
 
            near_dry = self.Bottom[3,pj,pi] # Check the value of this field not used in here
            
            c_here = ti.Vector([0.0, 0.0])
            c_here[0] =self.C[pj,pi][0]
            c_here[1] =self.C[pj,pi][1] 
            
            cW_east = self.C[pj,rightIdx][3]
            cS_north = self.C[upIdx,pi][2]
            
            phix = 0.5
            phiy = 0.5
            
            minH = min(h_vec.w, min(h_vec.z, min(h_vec.y, h_vec.x)))
            ...
            auxiliary = ti.Vector([0.0, 0.0, 0.0, 0.0],float)
            
            xflux[0] = NumericalFlux(aplus, aminus, hW_east * uW_east, h_here.y * u_here.y, mass_diff_x)
            xflux[1] = NumericalFlux(aplus, aminus, hW_east * uW_east**2 , h_here.y * u_here.y**2 , P_diff_x)
            ...
            yflux[2] = NumericalFlux(bplus, bminus, hS_north * vS_north**2, h_here.x * v_here.x**2, Q_diff_y)
            ...
            self.Auxiliary2[j,i] = [self.Auxiliary2[pj,pi][0], 0.0, self.Auxiliary2[pj,pi][2], breaking_white]
            ...
            if self.useSedTransModel==True :
                ...
                c1W_east = self.Sed_C1[pj,rightIdx][3]
                c1S_north = self.Sed_C1[upIdx,pi][2]
    
                c2_here[0] = self.Sed_C2[pj, pi][0]
                c2_here[1] = self.Sed_C2[pj, pi][1]
                c2W_east = self.Sed_C2[pj, rightIdx][3]
                c2S_north = self.Sed_C2[upIdx, pi][2]
    
                c3_here[0] = self.Sed_C3[pj, pi][0]
                c3_here[1] = self.Sed_C3[pj, pi][1]
                c3W_east = self.Sed_C3[pj, rightIdx][3]
                c3S_north = self.Sed_C3[upIdx, pi][2]
                
                c4_here[0] = self.Sed_C4[pj, pi][0]
                c4_here[1] = self.Sed_C4[pj, pi][1]
                c4W_east = self.Sed_C4[pj, rightIdx][3]
                c4S_north = self.Sed_C4[upIdx, pi][2]
                
                xflux_Sed = ti.Vector([0.0, 0.0, 0.0, 0.0],float)
                yflux_Sed = ti.Vector([0.0, 0.0, 0.0, 0.0],float)
                
                xflux_Sed[0] = NumericalFlux(aplus, aminus, hW_east * uW_east *c1W_east, h_here[1] * u_here[1] * c1_here[1], phix*(hW_east*c1W_east - h_here[1]*c1_here[1]))
                xflux_Sed[1] = NumericalFlux(aplus, aminus, hW_east * uW_east *c2W_east, h_here[1] * u_here[1] * c2_here[1], phix*(hW_east*c2W_east - h_here[1]*c2_here[1]))
                xflux_Sed[2] = NumericalFlux(aplus, aminus, hW_east * uW_east *c3W_east, h_here[1] * u_here[1] * c3_here[1], phix*(hW_east*c3W_east - h_here[1]*c3_here[1]))
                xflux_Sed[3] = NumericalFlux(aplus, aminus, hW_east * uW_east *c4W_east, h_here[1] * u_here[1] * c4_here[1], phix*(hW_east*c4W_east - h_here[1]*c4_here[1]))
           
                yflux_Sed[0] = NumericalFlux(bplus, bminus, hS_north *c1S_north * vS_north, h_here[0] *c1_here[0] * v_here[0], phiy *(hS_north*c1S_north - h_here[0]*c1_here[0]))
                yflux_Sed[1] = NumericalFlux(bplus, bminus, hS_north *c2S_north * vS_north, h_here[0] *c2_here[0] * v_here[0], phiy *(hS_north*c2S_north - h_here[0]*c2_here[0]))
                yflux_Sed[2] = NumericalFlux(bplus, bminus, hS_north *c3S_north * vS_north, h_here[0] *c3_here[0] * v_here[0], phiy *(hS_north*c3S_north - h_here[0]*c3_here[0]))
                yflux_Sed[3] = NumericalFlux(bplus, bminus, hS_north *c4S_north * vS_north, h_here[0] *c4_here[0] * v_here[0], phiy *(hS_north*c4S_north - h_here[0]*c4_here[0]))
    ...
    def Pass3(self):
        ...
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            ...
            in_state_here = self.State[pj, pi] # w, hu and hv, c (cell avgs, evaluated here)
            in_state_here_UV = self.StateUVstar[pj, pi] 
            ...
            if h_here <= h_cut:
                if h_north <= h_cut and h_east <= h_cut and h_south <= h_cut and h_west <= h_cut:
                    ...
                    self.current_stateUVstar[j,i] = ti.Vector([0.0, 0.0, 0.0, 0.0],float)
                    continue
            ...
            friction_ = FrictionCalc(in_state_here[1], in_state_here[2], h_here, self.maxdepth, self.delta, self.epsilon, self.isManning, self.g, friction_here)
            
            # Pressure stencil calculations
            P_left = self.ShipPressure[pj,pi-1].x
            P_right = self.ShipPressure[pj, pi+1].x
            P_down = self.ShipPressure[pj-1,pi].x
            P_up = self.ShipPressure[pj+1,pi].x
            ...
            C_state_here = self.State[pj, pi][3] 
            
            C_state_left = self.State[pj, pi-1][3] 
            C_state_right = self.State[pj, pi+1][3] 
            C_state_up = self.State[pj+1, pi][3] 
            C_state_down = self.State[pj-1, pi][3] 
            C_state_up_left = self.State[pj+1, pi-1][3] 
            C_state_up_right = self.State[pj+1, pi+1][3]         
            C_state_down_left = self.State[pj-1, pi-1][3] 
            C_state_down_right = self.State[pj-1, pi+1][3] 
            ...
            if(self.useBreakingModel == 1):
                breaking_B = self.Breaking[pj,pi].z  # breaking front parameter, non-breaking [0 - 1] breaking
            ...
            oldOldies = self.oldOldGradients[pj, pi]
            
            newState = ti.Vector([0.0, 0.0, 0.0, 0.0],float)   # w , hu, hv ,hc
            ...
            if self.timeScheme == 0: # if timeScheme is Euler do:
                ...
                newState = in_state_here_UV + self.dt/12. * (23.*d_by_dt - 16.*oldies + 5.*oldOldies)
                #Out = in_state_here + self.dt/12. * (23.*d_by_dt - 16.*oldies + 5.*oldOldies)
               
            elif self.pred_or_corrector == 2: # if time scheme is corrector
                # check this line, in the Pat's models this is saved in other vector
                predicted = self.predictedGradients[pj, pi]
                # corrector step
                newState = in_state_here_UV + self.dt/24. * (9.*d_by_dt + 19.*predicted - 5.*oldies + 1.*oldOldies)
            ...
            F_G_vec = ti.Vector([0.0, 0.0, 0.0, 1.0],float)
    ...
    def Pass3_SedTrans(self):
        ...
        for i,j in ti.ndrange((2,self.nx-2),(2,self.ny-2)): 
            ...
            # Calculate scalar transport additions
            C_state_here = self.State_Sed[pj, pi]
            
            C_state_left = self.State_Sed[pj, pi-1] 
            C_state_right = self.State_Sed[pj, pi+1] 
            
            C_state_up = self.State_Sed[pj+1, pi] 
            C_state_down = self.State_Sed[pj-1, pi] 
            
            C_state_up_left = self.State_Sed[pj+1, pi-1] 
            C_state_up_right = self.State_Sed[pj+1, pi+1] 
            
            C_state_down_left = self.State_Sed[pj-1, pi-1] 
            C_state_down_right = self.State_Sed[pj-1, pi+1] 
            ...
            if shields >= self.sedC1_criticalshields:
                erosion = self.sedC1_erosion * (shields - self.sedC1_criticalshields) * local_speed * divide_by_h
            
            
            Cmin = max(1.0e-6, C_state_here.x);   # only for C1 right now    
                
            deposition = min(2.0, (1.0 - self.sedC1_n) / Cmin) * C_state_here.x * self.sedC1_fallvel
            ...
            oldOldies = self.oldOldGradients_Sed[pj, pi]
            
            Out = ti.Vector([0.0, 0.0, 0.0, 0.0],float)   # w , hu, hv ,hc
    ...
    def Pass3Bous(self):
        ...
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            ...
            in_state_here = self.State[pj, pi] # w, hu and hv (cell avgs, evaluated here)
            in_state_here_UV = self.StateUVstar[pj, pi]
            ...
            if near_dry >0.0:
                ...
                F_G_star_oldOldies = self.F_G_star_oldOldGradients[pj,pi].xyz
                ...
                Psi1x = self.Bcoef_g * d3_here * ((eta_right_right - 2.0 * eta_right + 2.0 * eta_left - eta_left_left) *\
                                                  (0.5 * self.one_over_d3x) + \
                                                  (eta_up_right - eta_up_left - 2.0 * eta_right + 2.0 * eta_left + eta_down_right - eta_down_left) *\
                                                  (0.5 * self.one_over_dx * self.one_over_d2y))
                Psi2x = self.Bcoef_g * d2_here * (dd_by_dx * (2.0 * eta_by_dx_dx + eta_by_dy_dy) + dd_by_dy * eta_by_dx_dy) + \
                                                    (F_star - F_G_star_oldOldies.y) / self.dt * 0.5
                Psi1y = self.Bcoef_g * d3_here * ((eta_up_up - 2.0 * eta_up + 2.0 * eta_down - eta_down_down) * (0.5 * self.one_over_d3y) + \
                                                  (eta_up_right + eta_up_left - 2.0 * eta_up + 2.0 * eta_down - eta_down_right - eta_down_left) * \
                                                  (0.5 * self.one_over_dx * self.one_over_d2x))
                Psi2y = self.Bcoef_g * d2_here * (dd_by_dy * (2.0 * eta_by_dy_dy + eta_by_dx_dx) + dd_by_dx * eta_by_dx_dy) +\
                                                 (G_star - F_G_star_oldOldies.z) / self.dt * 0.5

            friction_here =  max(self.friction, self.BottomFriction[pj,pi].x)
            friction_ = FrictionCalc(in_state_here.y, in_state_here.z, h_here, self.maxdepth, self.delta, self.epsilon, self.isManning, self.g, friction_here)
            ...
            # Calculate scalar transport additions
            C_state_here = self.State[pj, pi].w
            
            C_state_left = self.State[pj, pi-1].w 
            C_state_right = self.State[pj, pi+1].w 
            C_state_up = self.State[pj+1, pi].w 
            C_state_down = self.State[pj-1, pi].w 
            C_state_up_left = self.State[pj+1, pi-1].w 
            C_state_up_right = self.State[pj+1, pi+1].w         
            C_state_down_left = self.State[pj-1, pi-1].w 
            C_state_down_right = self.State[pj-1, pi+1].w 
            ...
            if self.useBreakingModel==1:
                ...
                nu_flux_here = self.DissipationFlux[pj,pi]
            ...
            source_term = ti.Vector([overflow_dry, -self.g * h_here * detadx - in_state_here.y * friction_ + breaking_x + (Psi1x + Psi2x) + press_x, -self.g * h_here * detady - in_state_here.z * friction_ + breaking_y + (Psi1y + Psi2y) + press_y, hc_by_dx_dx + hc_by_dy_dy + 2.0 * hc_by_dx_dy + c_dissipation])

            d_by_dt = (xflux_west - xflux_here) * self.one_over_dx + (yflux_south - yflux_here) * self.one_over_dy + source_term

            newState = ti.Vector([0.0, 0.0, 0.0, 0.0],float)
            F_G_here = ti.Vector([0.0, F_star, G_star, 0.0],float)
    ...
    def Pass_Breaking(self,time:ti.f32):
        ...
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            ...
            P_south = self.State[pj-1,pi].y
            P_here = self.State[pj,pi].y
            P_north = self.State[pj+1,pi].y

            Q_west = self.State[pj,pi-1].z
            Q_here = self.State[pj,pi].z
            Q_east = self.State[pj,pi+1].z
            ...
            nu_breaking = min(1.0 * self.dx * self.dy / self.dt, B_Breaking * self.delta_breaking * h_here * detadt)
    ...
    def Pass_Dissipation(self):
        for i,j in ti.ndrange(self.nx,self.ny):
            if i >= self.nx - 2 or j >=self.ny-2 or i<=1 or j<=1:
                ...
                self.F_G_star[j,i]=ti.Vector([ 0.0, 0.0, 0.0, 0.0]) # 
                self.current_stateUVstar =ti.Vector([ 0.0, 0.0, 0.0, 0.0]) # txNewState StateUVstar  #check
                
                continue
            B_here = self.Bottom[j,i].z
            near_dry = self.Bottom[j,i].w 

            in_state_here = self.State[j,i]
            in_state_here_UV = self.StateUVstar[j,i]
            ...
            B_south = self.Bottom[j-1,i].z #check
            B_north = self.Bottom[j+1,i].z
            B_west = self.Bottom[j,i-1].z
            B_east = self.Bottom[j,i+1].z
            ...
            oldOldies = self.oldOldGradients[j,i]
            ...
            if near_dry>0.:
                ...
                F_G_star_oldOldies = self.F_G_star_oldOldGradients[j,i].xyz
                ...
                d_left_left = max(0.0, self.seaLevel - self.Bottom[j,i-2].z) #check
                ...
                Psi1y = self.Bcoef_g * d3_here * ((eta_up_up - 2.0 * eta_up + 2.0 * eta_down - eta_down_down) *\
                                                  (0.5 * self.one_over_d3y) + (eta_up_right + eta_up_left - 2.0 * eta_up + 2.0 *\
                                                                               eta_down - eta_down_right - eta_down_left) * \
                                                                                (0.5 * self.one_over_dx * self.one_over_d2x))
                Psi2y = self.Bcoef_g * d2_here * (dd_by_dy * (2.0 * eta_by_dy_dy + eta_by_dx_dx) + dd_by_dx * eta_by_dx_dy) +\
                                                 (G_star - F_G_star_oldOldies.z) / self.dt * 0.5
            ...
            f=0.0
            if self.isManning == True :
                f = self.g * pow(self.friction, 2.0) * ti.pow(ti.abs(divide_by_h), 1.0 / 3.0)
            else:
                f = self.friction / 2.0
            ...
            u = self.U[j,i]
            v = self.V[j,i]
            u_x = (u.y - u.w) * self.one_over_dx
            v_x = (v.y - v.w) * self.one_over_dx
            u_y = (u.x - u.z) * self.one_over_dy
            v_y = (v.x - v.z) * self.one_over_dy
            ...
            # Pressure stencil calculations
            P_left = self.ShipPressure[j,i-1].x
            P_right = self.ShipPressure[j,i+1].x
            P_down = self.ShipPressure[j-1,i].x
            P_up = self.ShipPressure[j+1,i].x

            press_x = -0.5 * h_here * self.g_over_dx * (P_right - P_left)
            press_y = -0.5 * h_here * self.g_over_dy * (P_up - P_down)

            # Calculate scalar transport additions
            C_state_here = self.State[j,i].w
            C_state_right = self.State[j,i+1].w
            C_state_left = self.State[j,i-1].w
            C_state_up = self.State[j+1,i].w
            C_state_down = self.State[j-1,i].w
            C_state_up_left = self.State[j+1,i-1].w
            C_state_up_right = self.State[j+1,i+1].w
            C_state_down_left = self.State[j-1,i-1].w
            C_state_down_right = self.State[j-1,i+1].w
            ...
            friction_ = FrictionCalc(in_state_here.y, in_state_here.z, h_here, self.maxdepth, self.delta, self.epsilon, self.isManning, self.g, friction_here)
            source_term = ti.Vector([overflow_dry, -self.g * h_here * detadx - in_state_here.y * friction_ + (Psi1x + Psi2x) + press_x, -self.g * h_here * detady - in_state_here.z * friction_ + (Psi1y + Psi2y) + press_y, hc_by_dx_dx + hc_by_dy_dy + 2.0 * hc_by_dx_dy + c_dissipation])

            d_by_dt = (xflux_west - xflux_here) * self.one_over_dx + (yflux_south - yflux_here) * self.one_over_dy + source_term
            newState = ti.Vector([0.0, 0.0, 0.0, 0.0])
            F_G_here = ti.Vector([0.0, F_star, G_star, 0.0])

            if self.timeScheme == 0 :
                newState = self.in_state_here_UV + self.dt * d_by_dt # check
            elif self.pred_or_corrector == 1 :
                newState = self.in_state_here_UV + self.dt / 12.0 * (23.0 * d_by_dt - 16.0 * oldies + 5.0 * oldOldies)
            elif self.pred_or_corrector == 2 :
                 predicted = self.predictedGradients[j,i]
                 newState = self.in_state_here_UV + self.dt / 24.0 * (9.0 * d_by_dt + 19.0 * predicted - 5.0 * oldies + oldOldies)

            # add passive tracer sources
            if (ti.max(ti.abs(detadx),ti.abs(detady)) * ti.sign(detadx * newState.y + detady * newState.z) > ti.dissipation_threshold) :
                newState.a = 1.0
    ...
    @ti.kernel
    def TriDiag_PCRx(self,p:int,s:int):
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            CurrentState = self.NewState[j,i]
            ...
            if p == 0:
                bIn = self.newcoef_x[j,i][1]
                bInLeft = self.newcoef_x[j,idx_left][1]
                bInRight = self.newcoef_x[j,idx_right][1]

                aIn = self.newcoef_x[j,i][0] / bIn
                aInLeft = self.newcoef_x[j,idx_left][0] / bInLeft
                aInRight = self.newcoef_x[j,idx_right][0] / bInRight

                cIn = self.newcoef_x[j,i][2] / bIn
                cInLeft = self.newcoef_x[j,idx_left][2] / bInLeft
                cInRight = self.newcoef_x[j,idx_right][2] / bInRight

                dIn = self.current_stateUVstar[j,i][1] / bIn  #check
                dInLeft = self.current_stateUVstar[j,idx_left][1] / bInLeft
                dInRight = self.current_stateUVstar[j,idx_right][1] / bInRight
                
            else:
                aIn = self.newcoef_x[j,i][0]
                aInLeft = self.newcoef_x[j,idx_left][0]
                aInRight = self.newcoef_x[j,idx_right][0]

                cIn = self.newcoef_x[j,i][2]
                cInLeft = self.newcoef_x[j,idx_left][2]
                cInRight = self.newcoef_x[j,idx_right][2]

                dIn = self.newcoef_x[j,i][3]
                dInLeft = self.newcoef_x[j,idx_left][3]
                dInRight = self.newcoef_x[j,idx_right][3]
            ...
            self.txtemp_PCRx[j,i]  = ti.Vector([aOut, 1.0, cOut, dOut])
            self.txtemp2_PCRx[j,i] = ti.Vector([CurrentState[0], dOut, CurrentState[2], CurrentState[3]])


    @ti.kernel
    def TriDiag_PCRy(self,p:ti.i32,s:ti.i32):
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            CurrentState = self.NewState[j,i]
            #CurrentState = self.current_stateUVstar[j,i]
            ...
            if p == 0:
                bIn = self.newcoef_y[j,i][1]
                bInLeft = self.newcoef_y[idx_left,i][1]
                bInRight = self.newcoef_y[idx_right,i][1]

                aIn = self.newcoef_y[j,i][0] / bIn
                aInLeft = self.newcoef_y[idx_left,i][0] / bInLeft
                aInRight = self.newcoef_y[idx_right,i][0] / bInRight

                cIn = self.newcoef_y[j,i][2] / bIn
                cInLeft = self.newcoef_y[idx_left,i][2] / bInLeft
                cInRight = self.newcoef_y[idx_right,i][2] / bInRight

                dIn = self.current_stateUVstar[j,i][1] / bIn  #check
                dInLeft = self.current_stateUVstar[idx_left,i][1] / bInLeft
                dInRight = self.current_stateUVstar[idx_right,i][1] / bInRight
               
            else:
                aIn = self.newcoef_y[j,i][0]
                aInLeft = self.newcoef_y[idx_left,i][0]
                aInRight = self.newcoef_y[idx_right,i][0]

                cIn = self.newcoef_y[j,i][2]
                cInLeft = self.newcoef_y[idx_left,i][2]
                cInRight = self.newcoef_y[idx_right,i][2]

                dIn = self.newcoef_y[j,i][3]
                dInLeft = self.newcoef_y[idx_left,i][3]
                dInRight = self.newcoef_y[idx_right,i][3]
            ...
            self.txtemp_PCRy[j,i] = ti.Vector([aOut, 1.0, cOut, dOut])
            self.txtemp2_PCRy[j,i] = ti.Vector([CurrentState[0],CurrentState[1], dOut, CurrentState[3]])
    ...
    def Run_Tridiag_solver(self):
        if self.model == 'SWE':
            ...
            Px = int(ti.ceil(ti.log(self.nx) / ti.log(2)))
            Py = int(ti.ceil(ti.log(self.ny) / ti.log(2)))
    ...
    def Update_Tridiag_coef(self):
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            ...
            if i <= 2 or i >= self.nx - 3 or near_dry < 0.0:
                a = 0.0
                b = 1.0
                c = 0.0
            else:
                ...
                a = d_here * d_dx / (6.0 * self.dx) - (self.Bcoef + 1.0 / 3.0) * d_here * d_here / (self.dx * self.dx)
                b = 1.0 + 2.0 * (self.Bcoef + 1.0 / 3.0) * d_here * d_here / (self.dx * self.dx)
                c = -d_here * d_dx / (6.0 * self.dx) - (self.Bcoef + 1.0 / 3.0) * d_here * d_here / (self.dx * self.dx)
        
            coefx = ti.Vector([a, b, c, 0.0])

            # Y-coefs
            coefy = ti.Vector([0.0, 0.0, 0.0, 0.0])
            if j <= 2 or j >= self.ny - 3 or near_dry < 0.0:
                a = 0.0
                b = 1.0
                c = 0.0
            else:
                ...
                # Calculate coefficients based on the depth and its derivative
                a = d_here * d_dy / (6.0 * self.dy) - (self.Bcoef + 1.0 / 3.0) * d_here * d_here / (self.dy * self.dy)
                b = 1.0 + 2.0 * (self.Bcoef + 1.0 / 3.0) * d_here * d_here / (self.dy * self.dy)
                c = -d_here * d_dy / (6.0 * self.dy) - (self.Bcoef + 1.0 / 3.0) * d_here * d_here / (self.dy * self.dy)
    ...
    @ti.kernel
    def Update_neardry(self):
        lengthCheck = 3  # check within three points
        for i,j in ti.ndrange((0,self.nx),(0,self.ny)):
            B_here = self.txBottom[j,i]
            # Update neardry
            B_here[3] = 99.0
            for yy in range(j - lengthCheck, j + lengthCheck + 1):
                for xx in range(i - lengthCheck, i + lengthCheck + 1):
                    xC = min(self.nx - 1, max(0, xx))
                    yC = min(self.ny - 1, max(0, yy))

                    bathy_C = self.Bottom[yC, xC][2]
                    if bathy_C >= 0.0:
                        B_here[3] = -99.0

            B_south = self.Bottom[j-1,i][2]
            B_north = self.Bottom[j+1,i][2]
            B_west = self.Bottom[j,i-1][2]
            B_east = self.Bottom[j,i+1][2]
