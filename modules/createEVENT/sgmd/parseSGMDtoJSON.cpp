...
parseGM_CSV(const std::string& filename) {
...
void writeJSON(std::string inputFilename, std::string outputFilename,double elev) {

  // parse inputfile
  std::vector<DataPoint> data = parseGM_CSV(inputFilename);  
...
int main(int argc, const char **argv) {
  ...
  while (std::getline(inputFile, line)) {
    ...
    //ss >> stationName >> comma >> lat >> comma >> lng >> comma >> elev;

    // write data to EventGrid file
    eventFile << stationName << "," << lng << "," <<  lat << "\n";
    ...
    writeJSON(inputCSVFilename, outputJSONFilename,elev);
