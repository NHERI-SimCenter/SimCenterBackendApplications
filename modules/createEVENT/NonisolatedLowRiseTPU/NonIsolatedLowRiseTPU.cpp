...
main(int argc, char **argv) {
  ...
  json_array_foreach(inputEventsArray, index, inputEvent) {
    ...
    std::cerr << "EVENT_TYPE: " << eventType << "\n";
    
    if (strcmp(eventType,"NonIsolatedLowRiseTPU") != 0) {
      
      json_array_append(outputEventsArray, inputEvent); 
      
    } else {
      std::cerr << "NONISOLATED\n";
      // create output event
      json_t *outputEvent = json_object();
      json_object_set(outputEvent,"type", json_string("Wind"));
      json_object_set(outputEvent, "subtype", json_string("NonIsolatedLowRiseTPU"));
...
int addEvent(json_t *generalInfo, json_t *currentEvent, json_t *outputEvent, bool getRV) {
	...
	return -2;        
      ...
      json_error_t error;
      json_t *tapData = json_load_file("tmpSimCenterNonIoslatedLowRiseTPU.json", 0, &error);
	...
	json_array_append(timeSeriesArray,timeSeriesRZ);
      ...
      const char *incidenceAngle = json_string_value(json_object_get(currentEvent,"incidenceAngle"));
      const char *roofType = json_string_value(json_object_get(currentEvent,"roofType"));
      const char *arrangement = json_string_value(json_object_get(currentEvent,"arrangement"));
      const char *density = json_string_value(json_object_get(currentEvent,"density"));
      const char *heightBreadth = json_string_value(json_object_get(currentEvent,"heightBreadth"));


      std::cerr << "CALLING callNonIsolatedLowRiseTPU\n";
      
      callNonIsolatedLowRise_TPU(arrangement, heightBreadth, density, roofType, incidenceAngle, "tmpSimCenterNonIsolatedLowRiseTPU.mat");
      
      //
      // invoke python and NonIsloatedLowRiseTPU.py script to process the .mat file into json file
      //
      
      // so instead invoke a process
      std::string pyArgs = "NonIsolatedLowRiseTPU.py tmpSimCenterNonIsolatedLowRiseTPU.mat tmpSimCenterNonIoslatedLowRiseTPU.json";
