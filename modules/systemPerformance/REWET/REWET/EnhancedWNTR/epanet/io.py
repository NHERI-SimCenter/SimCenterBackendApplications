    ...
    def _write_controls(self, f, wn):
        def get_setting(control_action, control_name):
            ...
            if attribute == 'status':
                ...
                control_action._target_obj, Valve
            ):
        ...
        for text, all_control in wn.controls():
            ...
            if all_control.epanet_control_type is not _ControlType.rule:
                ...
                if not isinstance(control_action.target()[0], Link):
                    continue
                if isinstance(
                    all_control._condition, (SimTimeCondition, TimeOfDayCondition)
                ):
...
class _EpanetRule(object):
    ...
    def parse_rules_lines(
        cls, lines, flow_units=FlowUnits.SI, mass_units=MassUnits.mg
    ) -> list:
        rules = list()
        rule = None
        in_if = False
...
class BinFile(object):
    ...
    #    @run_lineprofile()
    def read(
        self, filename, convergence_error=False, darcy_weisbach=False, convert=True
    ):
        ...
        with open(filename, 'rb') as fin:
            ...
            if convert:
                ...
                headloss[:, linktype < 2] = to_si(
                    self.flow_units, headloss[:, linktype < 2], HydParam.HeadLoss
                )  # Pipe or CV
                headloss[:, linktype >= 2] = to_si(
                    self.flow_units, headloss[:, linktype >= 2], HydParam.Length
                )  # Pump or Valve
