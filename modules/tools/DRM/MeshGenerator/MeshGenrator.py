...
def DRM_PML_Foundation_Meshgenrator(meshinfo) :
    ...
    if HaveFoundation == "YES":
        for i,block in enumerate(foundationBlocks):
            ...
            if i == 0:
                ...
                foundation = foundation.merge(pv.StructuredGrid(xBLOCK, yBLOCK, zBLOCK),merge_points=False,tolerance=1e-6,progress_bar = False)
    ...
    if HaveFoundation == "YES" and HavePiles == "YES":
        foundation2 = foundation.merge(piles,merge_points=False,tolerance=1e-6,progress_bar = False)
    ...
    if HaveFoundation == "YES" :
        ...
        if AttachFoundation == "YES":
            mesh = mesh.merge(foundation,merge_points=True,tolerance=1e-6,progress_bar = False)
        else:
            mesh = mesh.merge(foundation,merge_points=False,tolerance=1e-6,progress_bar = False)
    ...
    reg.cell_data['partitioned'][regularvtkOriginalCellIds] = regular.cell_data['partitioned']
    reg.cell_data['partitioned'][DRMvtkOriginalCellIds] = DRM.cell_data['partitioned'] + reg_num_cores
    ...
    if DRMinformation["DRM_Provider_Software"].lower() == "shakermaker" and DRMinformation["DRM_Location"].lower() == "local":
        pts = pts - xyz0
        pts[:,[0,1]] = pts[:,[1,0]]
        pts[:,2] = -pts[:,2]
    ...
    if DRMinformation["DRM_Location"].lower() == "local":
        ...
        if issubset:
            ...
            print("Please check the DRM nodes in the loading file or change the DRM mesh")
    else :
