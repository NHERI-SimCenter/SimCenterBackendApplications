...
def load_function_from_file(filepath, function_name):  # noqa: D103
...
def calculate_distances_with_direction(lat1, lon1, lat2, lon2):  # noqa: D103
    # Original points
    point1 = (lat1, lon1)
    point2 = (lat2, lon2)  # noqa: F841
...
def PlotSources(info):  # noqa: C901, N802, D103
    tmpLocation = info['tmpLocation']  # noqa: N806
    ...
    source_time_function = load_function_from_file(
        sourcetimeinfopath, 'source_time_function'
    )

    numFaults = len(faultinfo['Faultfilenames'])  # noqa: N806
    faultFiles = faultinfo['Faultfilenames']  # noqa: N806
    ...
    if numFaults < 1:
        print('No faults to plot')  # noqa: T201
        return

    if numFaults != len(faultFiles):
        print('Number of files does not match number of faults')  # noqa: T201
        return

    meshlist = []

    for i in range(numFaults):
        faultFile = tmpLocation + '/fault/' + faultFiles[i]  # noqa: N806
        sources = json.load(open(faultFile))  # noqa: SIM115, PTH123
    ...
    Mesh = meshlist[0]  # noqa: N806
    for i in range(1, numFaults):
        Mesh = Mesh.merge(meshlist[i])  # noqa: N806
    ...
    # create an arrow
    arrow = pv.Arrow(
        start=(hypocenter[0], hypocenter[1], ztail),
        direction=[0, 0, 1],
        scale=(zhead - ztail),
    )
    ...
    if not os.path.exists(tmpLocation):  # noqa: PTH110
        os.makedirs(tmpLocation)  # noqa: PTH103
    pl.link_views()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--tmplocation', help='output location for DRM', required=True
    )
    ...
    parser.add_argument(
        '--stationcoordinates', help='Station coordinates', required=False
    )
    ...
    if (
        ...
        and args.stationcoordinates
    ):
        # delete the first three characters
        stringcoords = args.stationcoordinates[:]
        info['stationCoordinates'] = [
            [float(i) for i in j.split(',')] for j in stringcoords.split(';')[:-1]
        ]

    if (
        info['plottingStation'].lower() in ['yes', 'true']
        and not args.stationcoordinates
    ):
        print('Station coordinates are required')  # noqa: T201
        exit()  # noqa: PLR1722

    if (
        info['plottingStation'].lower() in ['yes', 'true']
        and len(info['stationCoordinates']) != info['numstations']
    ):
        print('Number of stations does not match number of coordinates')  # noqa: T201
        exit()  # noqa: PLR1722

    if info['plotlayers'].lower() == 'yes' and not args.thickness:
        print('Thickness of layers is required')  # noqa: T201
        exit()  # noqa: PLR1722

    if info['plotlayers'].lower() == 'yes' and len(info['thickness']) < 1:
        print('At least one layer is required')  # noqa: T201
        exit()  # noqa: PLR1722
