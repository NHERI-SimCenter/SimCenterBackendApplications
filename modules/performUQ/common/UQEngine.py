...
def get_folder_names(run_index, simulation_number):
    if run_index == 0:
        templateDirName = "templatedir"
        workdirName = f"workdir.{simulation_number + 1}"
    else:
        templateDirName = f"templatedir.{run_index}"
        workdirName = f"workdir.{run_index}.{simulation_number + 1}"
    return templateDirName, workdirName


def get_sample_shape(sample_value):
    num_samples = np.shape(sample_value)[0]
    sample_dimension = np.shape(sample_value)[1]
    return num_samples, sample_dimension


def evaluate_model(sample_value, simulation_number, rv_name, work_dir, workflowDriver, runIdx=0):

    templatedirFolder, workdirFolder = get_folder_names(run_index=runIdx, simulation_number=simulation_number)

    sample_value = np.atleast_2d(sample_value)
    num_samples, sample_dimension = get_sample_shape(sample_value=sample_value)
    ...
    outF = open(current_dir_i + "/params.in", "w")
    outF.write("{}\n".format(sample_dimension))
    for i in range(sample_dimension):
        outF.write("{} {}\n".format(rv_name[i], sample_value[0, i]))
    outF.close()

    if runIdx == 0:
        print("RUNNING FEM: working directory {} created".format(simulation_number + 1))
    else:
        print("RUNNING FEM: working directory {}-{} created".format(runIdx, simulation_number + 1))
