...
if __name__ == "__main__":
    ...
    print(os.path.join(workdirMain, "logFileTMCMC.txt"), "w")
    ...
    dakotaJsonLocation = os.path.join(os.path.abspath(workdirTemplate), inputFile)
    logFile.write("\n\n==========================")
    ...
    (
        ...
        edpNamesList,
        edpLengthsList,
    ) = parseDataFunction(dakotaJsonLocation, logFile, workdirMain, mainscriptDir)
    ...
    # move cal data file
    os.rename(
        os.path.join(workdirTemplate, calDataFileName),
        os.path.join(workdirMain, calDataFileName),
    )
                ...
                if len(words) == lineLength:
                    ...
                    logFile.write(
                        ...
                        "does not match the expected length {}".format(
                            len(words), linenum, calDataFile, lineLength
                        )
                    )
    ...
    calibrationData = np.atleast_2d(
        np.genfromtxt(
            tempCalDataFile, skip_header=1, usecols=np.arange(2, 2 + lineLength)
        )
    )
    # logFile.write(
