...
#     #     -> tuple[Union[str, NDArray], int]:
#     #     sample_values = np.atleast_2d(sample_values)
#     #     msg = self._check_size_of_sample(sample_values)
#     #     if msg != "0": return msg, simulation_number
#     #     msg, workdir = self._create_workdir(simulation_number)
#     #     if msg != "0": return msg, simulation_number
#     #     msg = self._create_params_file(sample_values, workdir)
#     #     if msg != "0": return msg, simulation_number
#     #     msg = self._execute_driver_file(workdir)
#     #     if msg != "0": return msg, simulation_number
...
# def check_size_of_sample(sample_values, num_rv) -> str:
...
#         if num_values_in_each_sample != num_rv:
#             msg = f"Expected {num_rv} values in each sample, found {num_values_in_each_sample} in {sample_values}."
#             return msg
    
#     return "0"


# def create_workdir(full_path_of_tmpSimCenter_dir, simulation_number, list_of_dir_names_to_copy_files_from) -> tuple[str, str]:
#     dst = os.path.join(full_path_of_tmpSimCenter_dir, f"workdir.{simulation_number + 1}")
...
#                 except:
#                     msg = f"Could not remove file {file} from {dst}."
#                     return dst, msg
#             for dir in dirs:
#                 try:
#                     shutil.rmtree(os.path.join(root, dir))
#                 except:
#                     msg = f"Could not remove directory {dir} from {dst}."
#                     return dst, msg

#     for src_dir in list_of_dir_names_to_copy_files_from:
#         src = os.path.join(full_path_of_tmpSimCenter_dir, src_dir)
#         msg = copytree(src, dst)
#         if msg != "0":
#             return dst, msg
#     return dst, "0"


# def create_params_file(num_rv, list_of_rv_names, sample_values, dst) -> str:
#     list_of_strings_to_write = []
#     list_of_strings_to_write.append(f"{num_rv}")
#     for i, rv in enumerate(list_of_rv_names):
#         list_of_strings_to_write.append(f"{rv} {sample_values[0][i]}")
#     try: 
#         with open(os.path.join(dst, "params.in"), "w") as f:
#             f.write("\n".join(list_of_strings_to_write))
#     except Exception as ex:
#         return f"Failed to create params.in file. The following error occurred: \n{ex}"
#     return "0"


# def check_results(dst) -> str:
#     if glob.glob("results.out"):
#         outputs = np.loadtxt("results.out").flatten()
#     else:
#         msg = f"Error running FEM: results.out missing at {dst}"
...
# def evaluate_model_once(simulation_number: int, sample_values: NDArray, num_rv: int, 
#                         full_path_of_tmpSimCenter_dir: str, list_of_dir_names_to_copy_files_from: list[str], 
#                         list_of_rv_names: list[str], driver_filename: str) -> tuple[str, int]:
#     #
#     # 0. check size of sample
#     #
#     msg = check_size_of_sample(sample_values, num_rv)
#     if msg != "0":
#         return msg, simulation_number
    
#     #
#     # 1. Create workdir.simulation_number
#     #
#     dst, msg = create_workdir(full_path_of_tmpSimCenter_dir, simulation_number, list_of_dir_names_to_copy_files_from)
#     if msg != "0":
#         return msg, simulation_number
    
#     #
#     # 2. write params.in file
#     #
#     msg = create_params_file(num_rv, list_of_rv_names, sample_values, dst)
#     if msg != "0":
#         return msg, simulation_number
...
#     #
#     os.chdir(dst)
#     command = f"{os.path.join(dst, driver_filename)} 1> model_eval.log 2>&1"
...
#         return f"\n\n".join(returnStringList), simulation_number
#     #
#     # 4. read results.out
#     #
#     msg = check_results(dst)
#     if msg != "0":
#         return msg, simulation_number

#     os.chdir("../")

#     return "0", simulation_number