...
if pltm == 'linux' or pltm == 'linux2':
    c_lib = CDLL(
        os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'lib/linux/PLoM_C_library.so'
        )
    )
elif pltm == 'darwin':
    ...
    c_lib = CDLL(
        os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'lib/win/PLoM_C_library.so'
        )
    )
...
def L(
    y, g_c, x_mean, eta, s_v, hat_s_v, mu, phi, psi, lambda_i, D_x_g_c
