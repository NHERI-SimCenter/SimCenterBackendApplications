...
class surrogate(UQengine):
    ...
    def readJson(self):
        ...
        if (surrogateJson["method"] == "Sampling and Simulation") or (surrogateJson["method"] == "Import Data File"):
            self.do_mf = False
            self.modelInfoHF = model_info(surrogateJson, dakotaJson['randomVariables'], self.work_dir, x_dim, y_dim,
                                          self.errfile, self.n_processor, idx=0)
            self.modelInfoLF = model_info(surrogateJson, dakotaJson['randomVariables'], self.work_dir, x_dim, y_dim,
                                          self.errfile, self.n_processor, idx=-1)  # NONE model
        elif surrogateJson["method"] == "Import Multi-fidelity Data File":
            self.do_mf = True
            self.modelInfoHF = model_info(surrogateJson["highFidelity"], dakotaJson['randomVariables'], self.work_dir,
                                          x_dim, y_dim, self.errfile, self.n_processor, idx=1)
            self.modelInfoLF = model_info(surrogateJson["lowFidelity"], dakotaJson['randomVariables'], self.work_dir,
                                          x_dim, y_dim, self.errfile, self.n_processor, idx=2)
        else:
    ...
    def create_gp_model(self):
        ...
        if not self.do_mf:
            ...
            for i in range(y_dim):
                self.m_list = self.m_list + [
                    GPy.models.GPRegression(X_dummy, Y_dummy, kernel=kg.copy(), normalizer=True)]
        ...
        # for multi fidelity case
            ...
            X_list, Y_list = emf.convert_lists_to_array.convert_xy_lists_to_arrays([X_dummy, X_dummy],
                                                                                   [Y_dummy, Y_dummy])
                    ...
                    for parname in m_tmp.parameter_names():
                        if parname.endswith('lengthscale'):
                            if math.isnan(m_opt.log_likelihood()):
                                exec(
                                    'm_tmp.' + parname + '=np.random.exponential(1, (1, self.x_dim)) * init_length_params["' + parname.replace(
                                        '_', '.') + '"]')
                            else:
                                exec(
                                    'm_tmp.' + parname + '=np.random.exponential(1, (1, self.x_dim)) * m_opt.' + parname)
    ...
    def train_surrogate(self, t_init):
        ...
        self.X_lf, self.Y_lf = np.vstack([model_lf.X_existing, new_x_lf_tmp]), np.vstack(
            [model_lf.Y_existing, new_y_lf_tmp])
        ...
        for i in range(y_dim):
            self.set_XY(self.m_list[i], self.X_hf, self.Y_hf[:, i][np.newaxis].transpose(), self.X_lf,
                        self.Y_lf[:, i][np.newaxis].transpose())  # log-transform is inside set_XY
        ...
        while exit_flag == False:
            ...
            if self.do_logtransform:
                ...
                self.Y_cv_var = np.exp(2 * self.Y_cvs + self.Y_cv_vars) * (
                            np.exp(self.Y_cv_vars) - 1)  # in linear space
            else:
    ...
    def save_model(self, filename):
        ...
        np.savetxt(self.work_dir + '/surrogateTab.out', xy_sur_data, header=header_string_sur, fmt='%1.4e',
                   comments='%')
        ...
        for ny in range(self.y_dim):
            ...
            if not self.do_logtransform:
                results["yPredict_CI_lb"][self.g_name[ny]] = norm.ppf(0.25, loc=self.Y_cv[:, ny],
                                                                      scale=np.sqrt(self.Y_cv_var[:, ny])).tolist()
                results["yPredict_CI_ub"][self.g_name[ny]] = norm.ppf(0.75, loc=self.Y_cv[:, ny],
                                                                      scale=np.sqrt(self.Y_cv_var[:, ny])).tolist()
    ...
    def run_design_of_experiments(self, nc1, nq, e2, doeIdx="HF"):
        ...
        if self.doe_method == "none":
            ...
            for ni in range(self.cal_interval):
                ...
                if self.do_parallel:
                    iterables = (
                    (copy.deepcopy(m_stack), xc1[i, :][np.newaxis], xq, np.ones((nq, self.y_dim)), i, y_idx, doeIdx) for
                    i in range(nc1))
                    result_objs = list(self.pool.starmap(imse, iterables))
                    IMSEc1 = np.zeros(nc1)
                    for IMSE_val, idx in result_objs:
                        IMSEc1[idx] = IMSE_val
                    print("IMSE: finding the next DOE {} - parallel .. time = {:.2f}".format(ni,
                                                                                             time.time() - tmp))  # 7s # 3-4s
                    ...
                    IMSEc1 = np.zeros(nc1)
                    for i in range(nc1):
                        IMSEc1[i], dummy = imse(copy.deepcopy(m_stack), xc1[i, :][np.newaxis], xq,
                                                np.ones((nq, self.y_dim)), i, y_idx, doeIdx)
            ...
            for ni in range(self.cal_interval):
                ...
                tmp = time.time()
                if self.do_parallel:
                    iterables = ((copy.deepcopy(m_stack), xc1[i, :][np.newaxis], xq, phiqr, i, y_idx, doeIdx) for i in
                                 range(nc1))
                    result_objs = list(self.pool.starmap(imse, iterables))
                    IMSEc1 = np.zeros(nc1)
                    for IMSE_val, idx in result_objs:
                    ...
                    IMSEc1 = np.zeros(nc1)
                    for i in range(nc1):
                        IMSEc1[i], dummy = imse(copy.deepcopy(m_stack), xc1[i, :][np.newaxis], xq, phiqr, i, y_idx,
                                                doeIdx)
                        if np.mod(i, 200) == 0:
    ...
    def get_cross_validation_err(self):
        ...
        for ny in range(Y_hf.shape[1]):
            ...
            for ns in range(X_hf.shape[0]):
                ...
                m_tmp = self.set_XY(m_tmp, X_tmp, Y_tmp[:, ny][np.newaxis].transpose(), X_lf,
